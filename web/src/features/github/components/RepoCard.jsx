...
const RepoCard = ({ repo }) => {
  ...
  const updatedDate = updated_at ? new Date(updated_at).toLocaleDateString() : 'Unknown';
  ...
  const detailItems = [
    { icon: Star, label: 'Stars', value: formatCount(stargazers_count) },
    { icon: GitFork, label: 'Forks', value: formatCount(repo.forks_count) },
  ...
  const analyzerMetrics = [
    ...
    { label: 'Popularity', value: popularityScore, meta: `${formatCount(stargazers_count)} stars` },
  ...
  const timelineItems = [
    ...
    { label: 'Updated', value: formatDate(updated_at), meta: formatRelativeDate(updated_at) },
  ];
  ...
  return (
    <>
      <div
        ...
        className="group relative flex min-h-[220px] w-full cursor-pointer flex-col overflow-hidden rounded-lg border border-(--color-border) bg-(--color-surface) p-4 shadow-(--shadow-panel) transition duration-200 hover:-translate-y-0.5 hover:border-(--color-border-strong) hover:bg-(--color-surface-muted) focus:outline-none focus:ring-4 focus:ring-(--color-focus)"
        role="button"
        tabIndex={0}
        onClick={openDetails}
      ...
      >
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {/* <span className="inline-flex mb-1 items-center gap-2 rounded-full border border-(--color-border) bg-(--color-app-bg) px-2.5 py-1 text-[11px] font-semibold text-(--color-text-muted)">
              ...
              <div className="grid gap-5 p-5 lg:grid-cols-[minmax(0,1fr)_18rem]">
                ...
                <aside className="space-y-4">
                  <div className="rounded-md border border-(--color-border) bg-(--color-app-bg) p-4 text-center">
                    ...
                    >
                      <div className="grid size-[5.5rem] place-items-center rounded-full bg-(--color-app-bg)">
                        <div>
